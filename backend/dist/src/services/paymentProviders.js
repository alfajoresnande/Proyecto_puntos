...
exports.cancelMercadoPagoPendingResource = cancelMercadoPagoPendingResource;
...
function resolvePaymentChoice(raw) {
    ...
    if (raw.provider === "mercadopago") {
        if (raw.method === "qr")
            return { provider: "mercadopago", method: "qr" };
...
function normalizeQrMode(value) {
    if (value === "static" || value === "hybrid")
        return value;
    return "dynamic";
}
function addIsoDurationToNow(duration) {
...
async function getMercadoPagoQrOrder(orderId) {
    if (!MERCADOPAGO_ACCESS_TOKEN) {
...
async function mercadoPagoResourceRequest(path, init = {}) {
    if (!MERCADOPAGO_ACCESS_TOKEN) {
...
function resourceStatus(payload) {
    return String(payload.status ?? "unknown").trim().toLowerCase();
}
function mercadoPagoCancellationError(label, resourceId, response, payload) {
    return new Error(`Mercado Pago: no se pudo anular ${label} ${resourceId} (${mercadoPagoErrorDetail(payload, response.status)}).`);
}
async function cancelMercadoPagoPendingResource(rawResourceId) {
    const resourceId = String(rawResourceId).trim();
    if (!resourceId)
        throw new Error("Identificador de Mercado Pago vacio.");
    if (resourceId.toUpperCase().startsWith("ORD")) {
        ...
        if (!lookup.response.ok)
            throw mercadoPagoCancellationError("la order QR", resourceId, lookup.response, lookup.payload);
        ...
        const canceled = await mercadoPagoResourceRequest(`/v1/orders/${encodeURIComponent(resourceId)}/cancel`, {
            method: "POST",
            headers: { "X-Idempotency-Key": (0, crypto_1.randomUUID)() },
        });
    ...
    if (/^\d+$/.test(resourceId)) {
        ...
        if (!lookup.response.ok)
            throw mercadoPagoCancellationError("el pago", resourceId, lookup.response, lookup.payload);
        const currentStatus = resourceStatus(lookup.payload);
    ...
    if (!lookup.response.ok)
        throw mercadoPagoCancellationError("la preferencia", resourceId, lookup.response, lookup.payload);
