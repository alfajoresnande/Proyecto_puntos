...
const authCookiePolicy = (0, authCookie_1.resolveCookiePolicy)();
...
function loadDeviceCookieSecret() {
    ...
    if (jwtSecret) {
        if (!deviceSecretWarned) {
            ...
            console.warn("[auth] DEVICE_COOKIE_SECRET no configurado: se deriva una clave propia desde JWT_SECRET. " +
                "Configura DEVICE_COOKIE_SECRET con un valor aleatorio distinto para separar las claves.");
        }
        return Buffer.from(crypto_1.default.hkdfSync("sha256", Buffer.from(jwtSecret, "utf8"), Buffer.alloc(0), Buffer.from(DEVICE_SECRET_INFO), 32));
    }
...
function deviceCookieSecret() {
    if (!cachedDeviceSecret)
        cachedDeviceSecret = loadDeviceCookieSecret();
