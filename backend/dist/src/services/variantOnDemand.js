 ...
 */
function withCaseVariants(exts) {
...
function createVariantOnDemandMiddleware(uploadsDir) {
    return async function variantOnDemand(req, res, next) {
        ...
        if (originalMatch) {
            const originalPath = path_1.default.join(uploadsDir, filename);
            try {
                await fs_1.promises.access(originalPath);
                return next(); // el original existe, lo sirve el static
            }
            catch {
                const webpName = `${originalMatch[1]}.webp`;
                try {
                    await fs_1.promises.access(path_1.default.join(uploadsDir, webpName));
