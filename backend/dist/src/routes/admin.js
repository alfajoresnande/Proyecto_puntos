...
router.patch("/usuarios/:id/activo", async (req, res) => {
    ...
    const { affectedRows } = await (0, db_1.qRun)(db_1.pool, "UPDATE usuarios SET activo = ?, token_version = token_version + 1 WHERE id = ?", [activo ? 1 : 0, id]);
    if (affectedRows === 0) {
        res.status(404).json({ error: "Usuario no encontrado" });
        return;
    }
...
router.put("/configuracion/:clave", async (req, res) => {
    ...
    if (clave === "modo_venta" && ![salesMode_1.SALES_MODE_ECOMMERCE, salesMode_1.SALES_MODE_WHATSAPP].includes(String(valor))) {
...
router.get("/arrepentimiento", async (req, res) => {
    ...
    const items = await (0, db_1.qAll)(db_1.pool, `SELECT
       codigo_tramite,
       numero_orden,
       nombre_apellido,
       email,
