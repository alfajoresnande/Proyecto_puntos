...
function parseWhatsappOrderItems(value) {
...
function buildManualChargeWhatsappUrl(input) {
    ...
    if (!whatsappNumber || !input.checkoutUrl)
        return null;
...
router.get("/pedidos-whatsapp", async (_req, res) => {
    const rows = await (0, db_1.qAll)(db_1.pool, `SELECT id, estado, entrega, localidad, notas, moneda, subtotal_estimado,
...
router.patch("/pedidos-whatsapp/:id/estado", (0, requestRateLimit_1.requestRateLimit)({
    ...
    const parsed = zod_1.z.object({
        estado: zod_1.z.enum(["generado", "contactado", "cancelado"]),
    }).safeParse(req.body ?? {});
    ...
    const result = await (0, db_1.qRun)(db_1.pool, "UPDATE pedidos_whatsapp SET estado = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [parsed.data.estado, pedidoId]);
    if (!result.affectedRows) {
        res.status(404).json({ error: "Pedido de WhatsApp no encontrado." });
        return;
    }
    (0, realtime_1.emitRealtime)(["pedidos-whatsapp"]);
...
router.get("/cobros-manuales", async (req, res) => {
    ...
    const rows = await (0, db_1.qAll)(db_1.pool, `SELECT cm.id, cm.monto, cm.moneda, cm.concepto, cm.cliente_nombre, cm.cliente_telefono,
     ...
     LIMIT 50`, soloPropios ? [req.user.id] : []);
    res.json(rows.map((row) => {
 ...
 */
async function findCobroOperable(cobroId, user) {
    const cobro = await (0, db_1.qOne)(db_1.pool, `SELECT id, estado, monto, creado_por, oculto, preference_id, provider_payment_id
     FROM cobros_manuales
     WHERE id = ?
     LIMIT 1`, [cobroId]);
    if (!cobro)
        return null;
    if (user.rol === "vendedor" && Number(cobro.creado_por) !== user.id)
        return null;
    return cobro;
}
function parseCobroId(raw) {
    const id = Number(raw);
...
router.post("/cobros-manuales/:id/cancelar", async (req, res) => {
    ...
    if (recurso) {
        try {
            await (0, paymentProviders_1.cancelMercadoPagoPendingResource)(recurso);
        }
        catch (error) {
            const detalle = error instanceof Error ? error.message : "Error desconocido.";
            (0, securityMonitor_1.recordSecurityEvent)("cobro_manual_cancelacion_fallida", req, { cobroId, detalle });
    ...
    const updated = await (0, db_1.qRun)(db_1.pool, "UPDATE cobros_manuales SET estado = 'cancelado', error_mensaje = NULL WHERE id = ? AND estado <> 'aprobado'", [cobroId]);
    if (!updated.affectedRows) {
        res.status(409).json({ error: "El cobro se aprobo mientras se cancelaba. Revisalo en Mercado Pago." });
        return;
    }
    (0, securityMonitor_1.recordSecurityEvent)("cobro_manual_cancelado", req, { cobroId, monto: Number(cobro.monto) });
    (0, realtime_1.emitRealtime)(["cobros-manuales"]);
...
router.post("/cobros-manuales/:id/visibilidad", async (req, res) => {
    ...
    const parsed = zod_1.z.object({ oculto: zod_1.z.boolean() }).safeParse(req.body ?? {});
    if (!parsed.success) {
    ...
    await (0, db_1.qRun)(db_1.pool, "UPDATE cobros_manuales SET oculto = ? WHERE id = ?", [parsed.data.oculto ? 1 : 0, cobroId]);
    (0, realtime_1.emitRealtime)(["cobros-manuales"]);
...
function normalizeOrderMapAddress(value) {
...
router.get("/ordenes/mapa", async (req, res, next) => {
    try {
       ...
       ORDER BY o.created_at DESC, o.id DESC
       LIMIT 300`, includeSelectedOrder ? [selectedOrderId] : []);
        res.json(rows
            .map((row) => {
