...
async function ensureAuthProtectionSchema() {
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS auth_rate_limit_counters (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS auth_cooldowns (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS pending_registrations (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS password_reset_tokens (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
      usuario_id INT NOT NULL,
      token_hash CHAR(64) NOT NULL,
      expires_at DATETIME NOT NULL,
    ...
    if (!(await columnExists("password_reset_tokens", "attempts"))) {
        await exports.pool.query("ALTER TABLE password_reset_tokens ADD COLUMN attempts TINYINT UNSIGNED NOT NULL DEFAULT 0 AFTER used_at");
    }
    if (!(await columnExists("password_reset_tokens", "device_id"))) {
        await exports.pool.query("ALTER TABLE password_reset_tokens ADD COLUMN device_id CHAR(36) NULL AFTER requested_user_agent");
    }
}
...
async function ensureGlobalConfigurationSchema() {
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS configuracion (
    ...
    const defaultConfigs = [
        ...
        },
        {
            clave: "modo_venta",
            valor: "ecommerce",
            descripcion: "Modo de venta: ecommerce con stock y pagos, o catalogo_whatsapp para consultas manuales.",
        },
        {
            clave: "whatsapp_pedidos_numero",
            valor: "5493794632610",
            descripcion: "Numero de WhatsApp de pedidos, con codigo de pais y solo digitos.",
        },
        {
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS cobros_manuales (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    ...
    const [ocultoRows] = await exports.pool.query(`SELECT 1 FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'cobros_manuales' AND COLUMN_NAME = 'oculto'
     LIMIT 1`);
    if (!ocultoRows.length) {
        await exports.pool.query("ALTER TABLE cobros_manuales ADD COLUMN oculto TINYINT(1) NOT NULL DEFAULT 0 AFTER error_mensaje");
        await exports.pool.query("ALTER TABLE cobros_manuales ADD INDEX idx_cobros_manuales_oculto_created_at (oculto, created_at)");
    }
}
...
async function ensureProductosEcommerceSchema() {
    ...
    const [envioGratisRows] = await exports.pool.query(`SELECT 1 FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'productos' AND COLUMN_NAME = 'envio_gratis'
     LIMIT 1`);
    if (!envioGratisRows.length) {
        await exports.pool.query("ALTER TABLE productos ADD COLUMN envio_gratis TINYINT(1) NOT NULL DEFAULT 0 AFTER permite_envio");
    }
    ...
    if (!permiteRetiroRows.length) {
        await exports.pool.query("ALTER TABLE productos ADD COLUMN permite_retiro_local TINYINT(1) NOT NULL DEFAULT 1 AFTER envio_gratis");
    }
    ...
    const [imagenMobileRows] = await exports.pool.query(`SELECT 1 FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'productos' AND COLUMN_NAME = 'imagen_mobile_url'
     LIMIT 1`);
    if (!imagenMobileRows.length) {
        await exports.pool.query("ALTER TABLE productos ADD COLUMN imagen_mobile_url VARCHAR(255) NULL AFTER imagen_url");
    }
    ...
    const [destacadoHomeRows] = await exports.pool.query(`SELECT 1 FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'productos' AND COLUMN_NAME = 'destacado_home'
     LIMIT 1`);
    if (!destacadoHomeRows.length) {
        await exports.pool.query("ALTER TABLE productos ADD COLUMN destacado_home TINYINT(1) NOT NULL DEFAULT 0 AFTER puntaje_al_comprar");
    }
...
async function ensureOrderCoreSchema() {
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS pedidos_whatsapp (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
      usuario_id INT NULL,
      carrito_id BIGINT UNSIGNED NULL,
    ...
    try {
        const [statusRows] = await exports.pool.query(`SELECT COLUMN_TYPE AS column_type
       FROM information_schema.COLUMNS
...
async function ensurePricingDiscountSchema() {
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS descuentos_tipo_producto (
      id INT PRIMARY KEY AUTO_INCREMENT,
      tipo_cliente ENUM('cliente','mayorista','empleado') NOT NULL,
      producto_id INT NOT NULL,
      descuento_porcentaje DECIMAL(5,2) NOT NULL DEFAULT 0,
...
async function ensureSaboresSchema() {
    ...
    const indexExists = async (tableName, indexName) => {
        const [rows] = await exports.pool.query(`SELECT 1 FROM information_schema.STATISTICS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND INDEX_NAME = ?
       LIMIT 1`, [tableName, indexName]);
        return rows.length > 0;
    };
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS sabores (
      id INT PRIMARY KEY AUTO_INCREMENT,
      nombre VARCHAR(120) NOT NULL,
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS inventario_sabor_sucursal (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS producto_sabores (
      producto_id INT NOT NULL,
      sabor_id INT NOT NULL,
      orden TINYINT UNSIGNED NOT NULL DEFAULT 1,
      activo TINYINT(1) NOT NULL DEFAULT 1,
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS carrito_item_sabores (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS orden_item_sabores (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS movimientos_sabor_stock (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
      sabor_id INT NOT NULL,
      sucursal_id INT NULL,
      orden_id BIGINT UNSIGNED NULL,
    ...
    if (!(await columnExists("productos", "configuracion_tipo"))) {
        await exports.pool.query("ALTER TABLE productos ADD COLUMN configuracion_tipo ENUM('simple','caja_sabores') NOT NULL DEFAULT 'simple' AFTER tipo_producto");
    }
    if (!(await columnExists("productos", "capacidad_sabores"))) {
        await exports.pool.query("ALTER TABLE productos ADD COLUMN capacidad_sabores INT NULL AFTER configuracion_tipo");
    }
    if (!(await columnExists("carrito_items", "config_hash"))) {
        await exports.pool.query("ALTER TABLE carrito_items ADD COLUMN config_hash CHAR(64) NOT NULL DEFAULT '' AFTER modo_compra");
    }
    if (await indexExists("carrito_items", "uq_carrito_item_producto_modo")) {
        try {
            await exports.pool.query("ALTER TABLE carrito_items DROP INDEX uq_carrito_item_producto_modo");
        }
        catch {
    ...
    if (!(await indexExists("carrito_items", "uq_carrito_item_producto_modo_config"))) {
        await exports.pool.query("ALTER TABLE carrito_items ADD UNIQUE INDEX uq_carrito_item_producto_modo_config (carrito_id, producto_id, modo_compra, config_hash)");
    }
    if (!(await columnExists("orden_items", "config_hash"))) {
        await exports.pool.query("ALTER TABLE orden_items ADD COLUMN config_hash CHAR(64) NOT NULL DEFAULT '' AFTER modo_compra");
    }
    if (await indexExists("orden_items", "uq_orden_item_producto_modo")) {
        try {
            await exports.pool.query("ALTER TABLE orden_items DROP INDEX uq_orden_item_producto_modo");
        }
        catch {
            // Si el indice fue renombrado manualmente, seguimos y creamos el nuevo cuando sea posible.
        }
    ...
    if (!(await indexExists("orden_items", "uq_orden_item_producto_modo_config"))) {
        await exports.pool.query("ALTER TABLE orden_items ADD UNIQUE INDEX uq_orden_item_producto_modo_config (orden_id, producto_id, modo_compra, config_hash)");
    }
}
/**
...
async function ensurePagoWebhookEventosSchema() {
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS pago_webhook_eventos (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
      proveedor VARCHAR(40) NOT NULL,
...
async function ensureSessionRevocationSchema() {
    const [tokenVersionRows] = await exports.pool.query(`SELECT 1 FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'usuarios' AND COLUMN_NAME = 'token_version'
     LIMIT 1`);
    if (!tokenVersionRows.length) {
        await exports.pool.query("ALTER TABLE usuarios ADD COLUMN token_version INT UNSIGNED NOT NULL DEFAULT 0");
    }
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS sesiones_revocadas (
...
async function ensureSupportInboxSchema() {
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS soporte_conversaciones (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
      usuario_id INT NOT NULL,
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS soporte_mensajes (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    ...
    try {
        await exports.pool.query("ALTER TABLE soporte_conversaciones MODIFY estado ENUM('abierta','respondida','cerrada','archivada') NOT NULL DEFAULT 'abierta'");
    }
    catch {
...
async function ensureUbicacionesArgentinaSchema() {
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS argentina_localidades (
...
async function ensureAcreditacionPuntosSchema() {
    ...
    await exports.pool.query(`ALTER TABLE movimientos_puntos MODIFY COLUMN tipo ENUM('asignacion_manual','codigo_canje','referido_invitador','referido_invitado','canje_producto','devolucion_canje','acreditacion_compra','vencimiento_puntos','ajuste') NOT NULL`).catch(() => { });
    ...
    await exports.pool.query(`INSERT IGNORE INTO movimientos_puntos
     ...
     GROUP BY o.id, o.usuario_id`).catch(() => { });
    await exports.pool.query(`UPDATE usuarios u
     LEFT JOIN (
     ...
     WHERE u.puntos_saldo <> GREATEST(COALESCE(mp.saldo_calculado, 0), 0)`).catch(() => { });
}
async function ensurePointsLedgerSchema() {
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS puntos_lotes (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
      usuario_id INT NOT NULL,
    ...
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS puntos_lote_consumos (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
      usuario_id INT NOT NULL,
    ...
    const [cfgRows] = await exports.pool.query("SELECT valor FROM configuracion WHERE clave = 'puntos_vencimiento_meses' LIMIT 1");
    ...
    await exports.pool.query(`INSERT INTO puntos_lotes
    ...
    await exports.pool.query(`UPDATE usuarios u
     ...
     WHERE u.puntos_saldo <> GREATEST(COALESCE(pl.saldo_calculado, 0), 0)`).catch(() => { });
}
async function ensureCashOperationsSchema() {
    ...
    const [cancelledLocalSaleCashRows] = await exports.pool.query(`SELECT cm.caja_sesion_id, cm.referencia_id, cm.medio_pago, cm.monto, cm.creado_por
    ...
    for (const row of cancelledLocalSaleCashRows) {
        await exports.pool.query(`INSERT INTO caja_movimientos
...
async function ensureAppPresenceSchema() {
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS app_presencia_registros (
      id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
...
async function ensureLayoutTimelineSchema() {
    await exports.pool.query(`CREATE TABLE IF NOT EXISTS layout_timeline_eventos (
      id INT PRIMARY KEY AUTO_INCREMENT,
    ...
    const [rows] = await exports.pool.query("SELECT COUNT(*) as count FROM layout_timeline_eventos");
    if (rows[0].count === 0) {
        const defaultEvents = [
            {
                badge_text: "2025",
