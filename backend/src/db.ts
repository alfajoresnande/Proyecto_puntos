...
async function ensureCategoriasSchema() {
  ...
  if (!(await categoriaColumnExists("imagen_url"))) {
    await pool.query("ALTER TABLE categorias ADD COLUMN imagen_url VARCHAR(255) NULL");
  }
  if (!(await categoriaColumnExists("orden"))) {
    await pool.query("ALTER TABLE categorias ADD COLUMN orden INT NOT NULL DEFAULT 0");
  }
  if (!(await categoriaColumnExists("mostrar_en_home"))) {
    await pool.query("ALTER TABLE categorias ADD COLUMN mostrar_en_home TINYINT(1) NOT NULL DEFAULT 0");
  }
}
...
async function ensureLayoutTimelineSchema() {
  ...
  await pool.query("UPDATE layout_timeline_eventos SET imagen_url = '/rural-palermo.webp' WHERE titulo LIKE '%Palermo%' AND imagen_url LIKE '%alfajorescorrentinos.com%'");
  await pool.query("UPDATE layout_timeline_eventos SET imagen_url = '/came.webp' WHERE (titulo LIKE '%CAME%' OR titulo LIKE '%Corrientes en alfajores%') AND imagen_url LIKE '%alfajorescorrentinos.com%'");
  await pool.query("UPDATE layout_timeline_eventos SET imagen_url = '/lafalta.webp' WHERE titulo LIKE '%La Falda%' AND imagen_url LIKE '%alfajorescorrentinos.com%'");
}
...
async function ensurePaginasContenidoSchema() {
  ...
  for (const page of defaultPages) {
    await pool.query(
      `INSERT INTO paginas_contenido (slug, titulo, contenido)
       VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE slug = slug`,
      [page.slug, page.titulo, page.contenido]
    );
  }
}
