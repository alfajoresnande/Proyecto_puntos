...
type ShippingAddress = {
  ...
  envio?: ReturnType<typeof buildShippingQuoteSnapshot>;
  nombre?: string | null;
  telefono?: string | null;
...
function profileMissingFields(perfil?: PerfilCanje): string[] {
  if (!perfil) return ["nombre", "email", "dni", "telefono"];
  ...
  if (!perfil.telefono || perfil.telefono.replace(/\D/g, "").length < 6) missing.push("telefono");
...
async function validateProfileForCheckout(usuarioId: number): Promise<string[]> {
  const perfil = await qOne<PerfilCanje>(pool,
    "SELECT id, nombre, email, dni, telefono, fecha_nacimiento, localidad, provincia FROM usuarios WHERE id = ?",
    [usuarioId]
  );
...
async function lockActiveCartForItem(conn: Queryable, usuarioId: number, itemId: number): Promise<number | null> {
  const candidate = await qOne<{ carrito_id: number }>(
    conn,
  ...
  if (!candidate?.carrito_id) return null;

  const locked = await qOne<{ id: number }>(
    conn,
    `SELECT id
     FROM carritos
     ...
     LIMIT 1
     FOR UPDATE`,
    [Number(candidate.carrito_id), usuarioId],
  );
...
async function getCarritoItems(
  conn: Queryable,
  usuarioId: number,
  { forUpdate = false }: { forUpdate?: boolean } = {},
): Promise<CarritoItemDB[]> {
...
async function resolveCheckoutShippingAddress(
  ...
  rawAddress?: z.infer<typeof shippingAddressSchema> | null,
): Promise<ShippingAddress | null> {
  if (addressId) {
    const address = await getUserAddress(conn, usuarioId, addressId);
    if (!address) {
      throw new HttpError(404, "Direccion de envio no encontrada.");
    }
    return buildAddressSnapshot(address);
  }
  return rawAddress ? normalizeShippingAddress(rawAddress) : null;
}

async function resolveCheckoutShippingQuote(conn: Queryable, address: ShippingAddress): Promise<ShippingQuote> {
  const lat = Number(address.lat);
  const lng = Number(address.lng);
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
    throw new HttpError(400, "La direccion de envio no tiene coordenadas validas.");
  }
  const quote = await quoteShippingForCoordinates(conn, lat, lng);
...
function buildShippingAddressOrderSnapshot(address: ShippingAddress, quote: ShippingQuote): ShippingAddress {
  const quoteSnapshot = buildShippingQuoteSnapshot(quote);
  return {
    ...address,
...
function buildOrderReceiptMeta(
  order: { created_at: string; sucursal_retiro_id: number | null; direccion_envio_json?: string | null },
  ...
  const isShippingOrder =
    Boolean(direccionEnvio) &&
    typeof direccionEnvio === "object" &&
    (direccionEnvio as { metodo_entrega?: unknown }).metodo_entrega === "envio";
  return {
...
router.get("/canjes/:id", async (req, res) => {
  ...
  const row = await qOne<ClienteCanjeRow>(
    pool,
    `SELECT c.id, c.codigo_retiro, c.puntos_usados, c.estado, c.fecha_limite_retiro, c.notas, c.created_at,
            p.nombre AS producto_nombre, p.imagen_url AS producto_imagen,
            s.id AS sucursal_id, s.nombre AS sucursal_nombre, s.direccion AS sucursal_direccion,
  ...
  const itemsMap = await getCanjeItemsByCanjeIds(pool, [Number(row.id)]);
  const fallbackItem: CanjeItemDetalle = {
    producto_id: 0,
    producto_nombre: row.producto_nombre,
    producto_imagen: row.producto_imagen ?? null,
...
router.post(
  ...
  try {
    await conn.beginTransaction();
    if (!(await isWhatsappCatalogMode(conn))) {
    ...
    if (!carrito?.id) throw new HttpError(400, "Tu carrito esta vacio.");
    const carritoId = Number(carrito.id);
    const items = (await getCarritoItems(conn, usuarioId, { forUpdate: true })).filter(
      (item) => item.modo_compra === "dinero" && Number(item.carrito_id) === carritoId,
    );
    if (!items.length) throw new HttpError(400, "Tu carrito esta vacio.");
    ...
    let total = 0;
    const itemMessageGroups: string[] = [];
    ...
    const buildWhatsappMessage = (groups: string[], omittedItems: number) => [
    ...
    await qRun(conn, "UPDATE pedidos_whatsapp SET mensaje = ? WHERE id = ?", [message, pedidoWhatsappId]);
    await qRun(
      conn,
      "DELETE FROM carrito_items WHERE carrito_id = ? AND modo_compra = 'dinero'",
      [carritoId],
    );
    const remainingItems = await qOne<{ total: number }>(
      conn,
    ...
    await qRun(
      conn,
      Number(remainingItems?.total ?? 0) > 0
        ? "UPDATE carritos SET updated_at = CURRENT_TIMESTAMP WHERE id = ?"
        : "UPDATE carritos SET estado = 'convertido', updated_at = CURRENT_TIMESTAMP WHERE id = ?",
      [carritoId],
    );
    await conn.commit();
...
router.get("/checkout/shipping-quote", async (req, res) => {
  ...
  try {
    const address = await getUserAddress(pool, req.user!.id, direccionId);
    if (!address) {
...
router.post("/checkout/preview", async (req, res) => {
  const schema = z.object({
    ...
    direccion_id: z.number().int().positive().optional().nullable(),
  ...
  try {
    ...
    const direccionEnvio = metodoEntrega === "envio"
      ? await resolveCheckoutShippingAddress(
          conn,
          req.user!.id,
          parsed.data.direccion_id ?? null,
          parsed.data.direccion_envio ?? null,
        )
      : null;
...
router.post("/checkout/confirm", async (req, res) => {
  ...
  try {
    ...
    const totalDinero = toMoney(subtotalDineroProductos + costoEnvio);
    const totalPuntos = 0;
    const totalPuntosGanados = await calcularPuntosPorMonto(conn, totalDinero);
    ...
    await conn.commit();
    emitRealtime(estadoOrden === "borrador" ? ["inventario", "productos", "stats"] : ["ordenes", "inventario", "productos", "stats"]);
...
router.post("/checkout/ordenes/:id/process-payment", async (req, res) => {
  ...
  try {
    ...
    const orden = await qOne<{
      id: number;
      usuario_id: number;
      estado: "borrador" | "pendiente_pago" | "pagada" | "cancelada" | "expirada" | string;
      total_dinero: number;
      moneda: string;
...
router.post("/checkout/ordenes/:id/change-payment-method", async (req, res) => {
  ...
  try {
    ...
    const orden = await qOne<{
      ...
      conn,
      `SELECT o.id, o.usuario_id, o.estado, o.total_dinero, o.moneda, o.direccion_envio_json,
    ...
    await qRun(conn, "UPDATE pagos SET estado = 'rechazado' WHERE orden_id = ? AND estado = 'iniciado'", [ordenId]);
    await qRun(
      conn,
      `INSERT INTO pagos (
         orden_id, proveedor, metodo, estado, monto, comision_porcentaje, comision_monto, monto_neto,
...
router.get("/checkout/ordenes/:id/resume-payment", async (req, res) => {
  ...
  const orden = await qOne<{
    ...
    pool,
    `SELECT id, estado, total_dinero, direccion_envio_json,
            (SELECT COALESCE(SUM(cantidad * puntaje_al_comprar_unitario), 0)
             FROM orden_items
  ...
  const direccionEnvio = parseJsonField(orden.direccion_envio_json);
  const totalPuntosGanados = await calcularPuntosPorMonto(pool, Number(orden.total_dinero));
...
router.get("/checkout/ordenes/:id/payment-status", async (req, res) => {
  ...
  try {
    ...
    const orden = await qOne<{
      ...
      conn,
      `SELECT id, usuario_id, estado, total_dinero,
              (SELECT COALESCE(SUM(cantidad * puntaje_al_comprar_unitario), 0)
               FROM orden_items
               WHERE orden_id = ordenes.id AND modo_compra = 'dinero') AS total_puntos_ganados
       FROM ordenes
    ...
    const totalPuntosGanados = await calcularPuntosPorMonto(conn, Number(orden.total_dinero ?? 0));
