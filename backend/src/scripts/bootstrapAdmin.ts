...
function validatePassword(password: string): string | null {
  ...
  if (!/[a-z]/.test(password)) return "La contrasena debe incluir minusculas.";
  if (!/[A-Z]/.test(password)) return "La contrasena debe incluir mayusculas.";
  if (!/[0-9]/.test(password)) return "La contrasena debe incluir numeros.";
  if (!/[^A-Za-z0-9]/.test(password)) return "La contrasena debe incluir un simbolo.";
  return null;
}
...
async function main(): Promise<void> {
  ...
  const existente = await qOne<{ total: number }>(
    pool,
    "SELECT COUNT(*) AS total FROM usuarios WHERE rol IN ('admin','superAdmin')",
  );
  const yaHayAdmins = Number(existente?.total ?? 0) > 0;

  if (yaHayAdmins && !force) {
    console.error(
      "[admin:bootstrap] Ya existe al menos una cuenta administrativa. " +
  ...
  const usuario = await qOne<{ id: number }>(pool, "SELECT id FROM usuarios WHERE email = ?", [email]);
  ...
  if (usuario) {
    ...
    await qRun(pool, "UPDATE password_reset_tokens SET used_at = NOW() WHERE usuario_id = ? AND used_at IS NULL", [
      usuario.id,
    ]);
    ...
    const { insertId } = await qRun(
      pool,
      `INSERT INTO usuarios (nombre, email, email_verificado, email_verificado_at, password_hash, rol, activo)
       VALUES (?, ?, 1, NOW(), ?, ?, 1)`,
